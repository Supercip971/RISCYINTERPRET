...
public:
    ...
    inline uint16_t CSL_read_limm()
    { // funct3 with inter register operation
    ...
    inline uint16_t CSL_read_himm()
    { // funct3 with inter register operation
        return (raw_data & 0x1c00) >> 10;
        // 0001110000000000
    }
    inline uint16_t CISWLSBJ_read_funct3()
    { // yeah everything but no CR why ?
        return (raw_data & 0xe000) >> 13;
        // 1110000000000000
    }
    inline uint16_t CR_read_funct4()
    { // yeah everything but no CR why ?
    ...
    inline uint16_t CRISB_read_limm()
    {                                                // or rs2
